...
def find_authors_nationalities():
    ...
    for author in authors:
        result.append(f"{str(author)} is {author.nationality}")
...
def order_books_by_year():
    ...
    result = []
    for book in books:
        result.append(f"{book.publication_year} year: {str(book)}")

    return '\n'.join(result)


def delete_review_by_id(given_id):
    review = Review.objects.get(pk=given_id)
    review.delete()
...
def filter_authors_by_nationalities(nationality):
    ...
    result = []
    for author in authors:
        result.append(str(author) if not author.biography else author.biography)
...
def filter_authors_by_birth_year(start_year, end_year):
    authors = Author.objects.filter(birth_date__year__gte=start_year, birth_date__year__lte=end_year).order_by('-birth_date')
    result = []
    for author in authors:
        result.append(f'{author.birth_date}: {str(author)}')
